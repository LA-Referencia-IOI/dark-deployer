...
5. Generates a root `.env.integration` file with blockchain connection info, contract addresses, and the contract ABI extracted from `deployed_contracts.ini`.
...
The `.env.integration` file is the **handoff artifact**: it is generated automatically by each tier's install and must be copied manually to the next server before running its install. It carries all connection details (RPC URL, chain ID, contract addresses, master key) and the **contract ABI** extracted directly from `deployed_contracts.ini` — so if contracts are redeployed with an updated ABI, the new ABI flows to all services on the next install run without any manual intervention.
...
**Before running**, ensure the final `.env.integration` (produced after the storage install) is present at the project root.
...
[ ] 1. Blockchain server
        ...
        → .env.integration created at project root
